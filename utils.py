...
def random_crom_numpy_wrapper(height, width, x):
    return __RandomCropNumpy(x, (height, width))


def get_data_loader_folder(input_folder, batch_size, train, new_size=None,
                           height=256, width=256, num_workers=4, crop=True, conf=None):
    import functools
    wrapped_transform = functools.partial(random_crom_numpy_wrapper, height, width)
    transform_list = []
    transform_list = [transforms.ToTensor()] + transform_list
    transform_list = [transforms.Lambda(wrapped_transform)] + transform_list
